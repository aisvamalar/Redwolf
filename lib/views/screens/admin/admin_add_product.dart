...
class _AdminAddProductState extends State<AdminAddProduct> {
  ...
  void initState() {
    ...
    _specValueControllers = _specifications
        ...
        .toList();
    _keyFeatureControllers = _keyFeatures
  ...
  void _initializeWithProduct(AdminProduct product) {
    setState(() {
      ...
      _specifications =
          product.specifications
              ...
              .toList() ??
          [Specification(label: 'Dimension', value: '10×20×5 cm')];
  ...
  Future<void> _loadProductFiles(AdminProduct product) async {
    ...
    if (product.secondImageUrl != null &&
        ...
        (product.secondImageUrl!.startsWith('http://') ||
            product.secondImageUrl!.startsWith('https://'))) {
      try {
    ...
    if (product.glbFileUrl != null &&
        ...
        (product.glbFileUrl!.startsWith('http://') ||
            product.glbFileUrl!.startsWith('https://'))) {
      try {
  ...
  Future<void> _saveProduct(String status) async {
    ...
    final hasFirstImage =
        _productImages[0] != null ||
        (_isEditing &&
            widget.product != null &&
            (widget.product!.imageUrl.isNotEmpty));
    final hasSecondImage =
        _productImages[1] != null ||
        (_isEditing &&
            widget.product != null &&
            (widget.product!.secondImageUrl?.isNotEmpty ?? false));
    final hasThirdImage =
        _productImages[2] != null ||
        (_isEditing &&
            widget.product != null &&
    ...
    final hasValidSpecification = _specifications.any(
      (spec) => spec.label.trim().isNotEmpty && spec.value.trim().isNotEmpty,
    );
    ...
    final hasValidKeyFeature = _keyFeatures.any(
    ...
    try {
      final supabase = AdminSupabaseService();
      final productService = AdminProductService();
      ...
      final keyFeatures = _keyFeatures
      ...
      return SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 24),
            // Media
            _buildSectionCard('Media', [
              const Text(
                'Product Images (3 images required)',
                style: TextStyle(
                  fontSize: 14,
            ...
            _buildSectionCard(
              ...
              [
                ...
                ..._availableCategories.map(
                  (category) => _buildCheckbox(category),
                ),
                const SizedBox(height: 12),
                TextButton(
            ...
            const SizedBox(height: 24),
            // Specifications
            _buildSectionCard('Specifications', [
              ..._specifications.asMap().entries.map((entry) {
                final index = entry.key;
                final spec = entry.value;
                return Padding(
                  padding: EdgeInsets.only(
                    bottom: index < _specifications.length - 1 ? 12 : 0,
                  ),
            ...
            const SizedBox(height: 24),
            // Key Features
            _buildSectionCard('Key Features', [
              ..._keyFeatures.asMap().entries.map((entry) {
                final index = entry.key;
                return Padding(
                  padding: EdgeInsets.only(
                    bottom: index < _keyFeatures.length - 1 ? 12 : 0,
                  ),
